...
<script type="ts">
    ...
    $: profit = portfolioStats && (portfolioStats.unrealised_profit_usd + portfolioStats.realised_profit_usd) || 0;
		...
		<div class="card">
			...
			<ul class="list-group list-group-flush">
                <span class={determineProfitColourClass(portfolioStats.unrealised_profit_usd)}>
				    <li class="list-group-item">Unrealised profit: {formatDollar(portfolioStats.unrealised_profit_usd)}</li>
                </span>
			</ul>

			<ul class="list-group list-group-flush">
                <span class={determineProfitColourClass(portfolioStats.realised_profit_usd)}>
				    <li class="list-group-item">Realised profit: {formatDollar(portfolioStats.realised_profit_usd)}</li>
                </span>
			</ul>
